...
export default async function AdminPage() {
  ...
  const [invitations, totalProjects, totalPages, invitationDates] = await Promise.all([
    prisma.accessInvitation.findMany({ where: { status: "PENDING", expiresAt: { gt: new Date() } }, orderBy: { createdAt: "desc" }, take: 30, select: { id: true, email: true, name: true, expiresAt: true, createdAt: true } }),
  ...
  const pendingCount = await prisma.accessInvitation.count({ where: { status: "PENDING", expiresAt: { gt: new Date() } } });
